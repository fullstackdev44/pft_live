...
@Component({
  selector: 'pft-user-favorite',
  templateUrl: './user-favorite.component.html'
})
export class UserFavoriteComponent extends CommonComponent implements OnInit {
  ...
  removeVideoFromFavorites() {
    this.loading = true;
    this.user.favorite_videos.splice(this.index, 1);
    this.userService.updateUser(this.user, ["favorite_videos"]).subscribe(
      data => {
        this.loading = false;
