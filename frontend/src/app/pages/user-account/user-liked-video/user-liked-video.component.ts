...
@Component({
  selector: 'pft-user-liked-video',
  templateUrl: './user-liked-video.component.html'
})
export class UserLikedVideoComponent extends CommonComponent implements OnInit {
  ...
  removeVideoFromLikedVideos() {
    this.loading = true;
    this.user.liked_videos.splice(this.index, 1);
    this.userService.updateUser(this.user, ["liked_videos"]).subscribe(
      data => {
        this.loading = false;
