...
@Component({
  selector: 'pft-user-purchased-video',
  templateUrl: './user-purchased-video.component.html'
})
export class UserPurchasedVideoComponent extends CommonComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.initData(this.user, this.userService.getUserPurchasedVideo(logged_in_user._id));
  ...
  removeVideoFromPurchasedVideos() {
    this.loading = true;
    this.user.purchased_videos.splice(this.index, 1);
